...
export default function ModernHomePage() {
  ...
  const calculate = async () => {
    ...
    setError("");
    try {
      const response = await fetch("/api/estimate", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ areaSqm: computedArea }) });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || "We could not calculate the estimate.");
      setRecommendations(data.recommendations || []);
      setServices(data.services || []);
      setSelectedId(data.selectedProductId || "");
      setCalculated(true);
    } catch (err: any) {
      setError(err.message || "We could not calculate the estimate.");
    } finally {
      setLoading(false);
    }
  };
